...
import { accrueInvestmentAccountByUserId } from "../services/interestAccrual.service";
...
export async function getMe(req: Request, res: Response) {
  // Catch up any interest the nightly cron might have missed (container
  // restart, redeploy, cold start) before returning the balance — see
  // interestAccrual.service.ts for why this matters more than the cron.
  const account = await accrueInvestmentAccountByUserId(req.user!.id);
...
export async function invest(req: Request, res: Response) {
  ...
  // Accrue on the *pre-deposit* principal first — otherwise any days
  // that elapsed before this deposit would incorrectly earn interest on
  // money that wasn't in the account yet.
  await accrueInvestmentAccountByUserId(req.user!.id);
