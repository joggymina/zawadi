...
import { accrueLoans } from "../services/interestAccrual.service";
...
export async function listMine(req: Request, res: Response) {
  ...
  // Catch up interest on outstanding loans before showing the borrower
  // their balance — see interestAccrual.service.ts.
  return res.json(await accrueLoans(loans));
}
...
export async function listFunded(req: Request, res: Response) {
  ...
  // Catch up interest on each underlying loan before showing the funder
  // their stake — see interestAccrual.service.ts.
  const accruedLoans = await accrueLoans(fundings.map((f) => f.loan));
