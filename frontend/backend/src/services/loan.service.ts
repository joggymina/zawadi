...
import { accrueLoan } from "./interestAccrual.service";
...
export async function repayLoan(params: { loanId: string; borrowerId: string; amount: number }) {
  ...
  // Catch up any missed interest accrual *before* opening the
  // transaction below, so the interest-first/principal-second split (and
  // the resulting funder distribution) is always computed against the
  // true up-to-date interestOwed, not a stale cron snapshot. This runs
  // as its own standalone write, outside the transaction that follows —
  // calling accrueLoan from inside that transaction would have it write
  // through a different connection while the outer transaction still
  // holds the row, risking a lock wait against itself.
  const existingLoan = await prisma.loan.findUnique({ where: { id: params.loanId } });
  if (existingLoan) await accrueLoan(existingLoan);
