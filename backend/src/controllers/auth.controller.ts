...
export async function register(req: Request, res: Response) {
  ...
  const user = await prisma.user.create({
    data: {
      ...
      phoneNumber,
      passwordHash,
      account: { create: {} }, // zero-balance InvestmentAccount
    },
  });
...
export async function refresh(req: Request, res: Response) {
  ...
  // Rotate: revoke the old token, issue a new one. Prevents a stolen
  // refresh token from being replayed indefinitely.
  await prisma.refreshToken.update({ where: { id: record.id }, data: { revokedAt: new Date() } });
  await issueSession(res, record.user.id, record.user.role, record.user.username);

  return res.json({ id: record.user.id, username: record.user.username, role: record.user.role });
}
...
async function issueSession(res: Response, userId: string, role: "USER" | "ADMIN", username: string) {
  ...
  res.setHeader("X-Access-Token", accessToken); // client also reads this into memory
}
